<?php
// On va aller chercher les articles de la base de données
// On se connecte à la base
require_once 'check.php';

echo 	"<script language='JavaScript'>
				var clients = '';
				try {
					clients = localStorage.getItem('who', 'client');
				} catch {}
				if (clients == 'client') {
					
				} else {
					document.location.href='profil.php';
				}
			</script>";

?>
<!DOCTYPE html>
<html lang="en">

<head>
	<meta charset="UTF-8">
	<meta http-equiv="X-UA-Compatible" content="IE=edge">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<link rel="stylesheet" type="text/css" href="./style/style.css">
	<link rel="stylesheet" type="text/css" href="./style/styleMyPage.css">
	<title>Mystery</title>
</head>

<body>
	<!--La tête du site-->
	<div class="human">
		<header>
			<nav>
				<ul>
					<li class="seize">
						<a href="index.php">
							<img id="logo" src="./img/mistery-logo.jpg" alt='logo du site'>
						</a>
					</li>
					<li class="seize">
						<p id="men"><a href="men.html">T-Shirt</a></p>
					</li>
					<li class="seize">
						<p id="nouveaute"><a href="news.html">Nouveauté</a></p>
					</li>
					<li class="seize">
						<img class="panier" src="https://www.tonightsound.com/v2/wp-content/uploads/2012/03/TONIGHTSOUND_ACTUALITES_PANIER.jpg">
					</li>

					<li class="seize"><a href="my-page.php"><img id="profil" src="./img/profil.png"></a></li>
				</ul>
			</nav>
		</header>

		<!--Si l'utilisateur a déjà un compte alors on ouvre cette fênetre-->
		<div class="mon-compte" id="mon-compte-id">
			<div id="menu-du-compte">
				<ul>
					<a href="mes-comptes.html">
						<li>Mes Commandes</li>
					</a>
					<a href="parametres.php">
						<li id="li-parametre">Paramètres</li>
					</a>
					<a href="deconnexion.php">
						<li id="li-deconnexion">Deconnexion</li>
					</a>
				</ul>
			</div>

			<div id="mes-cordonnees">
				<div class="donnees-du-client">
					<div id="cercle"></div>
					<div style="font-size: 18px;">
						<?php
						if (isset($_COOKIE['my_email'])) {
							$session_email = unserialize($_COOKIE['my_email']);

							echo 'Votre email de session est le "' . $session_email . '"';

							// Récupérer les données de ce client exact
							$pseudo_client = $bdd->prepare("SELECT pseudo FROM `user` WHERE email = '{$session_email}' LIMIT 1");
							$pseudo_client->execute();
							$pseudo_client2 = $pseudo_client->fetchAll();

							foreach ($pseudo_client2 as $pseudo_clients) {
								$pseudo_clients =  implode(" ", $pseudo_clients);
								$pseudo_clients = substr($pseudo_clients, 0, strrpos($pseudo_clients, ' '));
								echo " et votre pseudo est '" . $pseudo_clients . "'.";
							}
						}
						?>
					</div>
				</div>
			</div>

			<!--
			<div id="avantage-membre">
				<h3>Avantages Membre</h3>
				<a href="news.html">
					<img src="https://goal24.uz/uploads/posts/2020-09/1600070196_92726d56b49506cacdd1ffc9a44da1695f00a7c407a53277988115.jpg" alt="solde img">
					<p>Soldes réservés aux Membres</p>
				</a>
			</div>-->
		</div>

		<!--Clique sur l'icône parametre-->
		<div id="deconection">
			<p>Voulez-vous vraiment vous <u style="color: #4e81bf;">déconnecter</u> ?</p>
			<button id="no-deconection">Non</button>
			<button id="yes-deconection">Oui</button>
		</div>

		<!--La Panier du site-->
		<div class="panierOuvert">
			<h2>Panier</h2>
			<p id="total-style">Prix total: <span class="total-price">0</span>€</p>
			<p id="panier-no-mess" style="font-size: 12px;">Votre panier est tristement vide.</p><br>

			<button class="payer">Valider mon panier</button>
		</div>

		<!--Le pied du site-->
		<div class="footer">
			<div class="footer-boite" id="nous-suivre">
				<span>
					<p class="titre-footer" id="nous-suivre-id">Nous suivre sur</p>
				</span>
				<div>
					<a href="https://www.tiktok.com"><img class="nous-suivre-img" src="https://www.pngmart.com/files/20/TikTok-Logo-PNG.png" alt="tiktok_logo"></a>
					<a href="https://www.instagram.com/mstry.xyz/"><img class="nous-suivre-img" src="https://www.pngmart.com/files/13/Insta-Logo-PNG-Pic.png" alt="Instagram_logo"></a>
				</div>
			</div>

			<div class="footer-boite" id="footer-compte">
				<span>
					<p class="titre-footer" id="compte-id">Compte</p>
				</span>
				<p><a href="profil.php">Mon Compte</a></p>
				<p><a href="mes-comptes.html">Mes Commandes</a></p>
				<p>Mon Panier</p>
			</div>

			<div class="footer-boite" id="soldes">
				<span>
					<p class="titre-footer" id="soldes-id">Magasin</p>
				</span>
				<p><a href="guide-taille.html">Size guide de taille</a></p>
				<p><a href="condition-de-nos-offres.html">Conditions de Nos Offres</a></p>
			</div>

			<div class="footer-boite" id="a-propos-de-nous">
				<span>
					<p class="titre-footer" id="a-propos-de-nous-id">A Propos de Nous
					<p></p>
				</span>
				<a href="tel: +33621939402"><img class="nous-suivre-img" src="data:image/png;base64," alt="telephone"></a>
				<a href="mailto:mystery33700@gmail.com"><img class="nous-suivre-img" src="data:image/png;base64," alt="mail"></a>
			</div>

			<p id='droit-d-auteur'>Droits d'auteur © 2022 - Tous les droits sont réservés. Juste Un TSHIRT wesh</p>

		</div>
		<script src="script.js"></script>
	</div>
</body>

</html>