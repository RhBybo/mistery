...
<head>
	...
	<link rel="stylesheet" type="text/css" href="./style/styleParametres.css">
...
<body>
	<div class="human">
		...
		<div class="body-parametres">
			...
			if (isset($_COOKIE['my_email'])) {
				...
				$pseudo_client = $bdd->prepare("SELECT pseudo FROM `user` WHERE email = '{$session_email}' LIMIT 1");
				$pseudo_client->execute();
				$pseudo_client2 = $pseudo_client->fetchAll();

				foreach ($pseudo_client2 as $pseudo_clients) {
					$pseudo_clients =  implode(" ", $pseudo_clients);
					$pseudo_clients = substr($pseudo_clients, 0, strrpos($pseudo_clients, ' '));
				}
			}
			?>
			<form action="parametre-mod.php" method="post">
				<br>
				<p style="font-size: 12px; color: gray;">Adresse e-mail</p>
				<input type="email" placeholder="" class="info-client" name="email-name">

				<br><br>
				<p style="font-size: 12px; color: gray;">Numéro de téléphone</p>
				<input type="text" placeholder="Ajouter votre numéro de téléphone" class="info-client" name="numero-name">

				<br><br>
				<p style="font-size: 12px; color: gray;">Adresse</p>
				<input type="text" placeholder="Ajouter votre adresse postal" class="info-client" name="adresse-name">

				<p id="modifier-mess">Veuillez cliquer sur le champ que vous voulez modifier</p>

				<div id="poche-fin">
					<button type="submit" name="modifierBtn" id="modifier">Modifier</button>
				</div>
			</form>
		...
		<!--Les mini-fênetres qui s'ouvrent pour modifier les info du client-->
		<div class="mod-page" id="email-mod-page">

		</div>

		<div class="mod-page" id="mdp-mod-page">

		</div>

		<div class="mod-page" id="numero-mod-page">

		</div>

		<div class="mod-page" id="adresse-mod-page">
	...
	<script type="text/javascript" src="parametres-js.js"></script>
	<script src="script.js"></script>
	<script>
		var my_var = <?php echo json_encode($session_email); ?>;
		document.getElementsByName('email-name')[0].placeholder = my_var;
	</script>
</body>
